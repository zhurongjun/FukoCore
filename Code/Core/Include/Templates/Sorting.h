...
{
	TArrayRange<T> ArrayRange(First, Num);
	::Fuko::Algo::IntroSort(ArrayRange, TDereferenceWrapper<T, TLess<T> >(TLess<T>()));
}
...
public:
	...
	static void Merge(T* First, const int32 Mid, const int32 Num, const PREDICATE_CLASS& Predicate)
	{
		...
		while (AStart < BStart && BStart < Num)
		{
			...
			int32 NewAOffset = (int32)::Fuko::Algo::Impl::UpperBoundInternal(First + AStart, BStart - AStart, First[BStart], FIdentityFunctor(), Predicate);
			AStart += NewAOffset;
			...
			int32 NewBOffset = (int32)::Fuko::Algo::Impl::LowerBoundInternal(First + BStart, Num - BStart, First[AStart], FIdentityFunctor(), Predicate);
